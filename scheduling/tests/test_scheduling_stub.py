...
def owner(db):
    ...
    user = User.objects.create_user(email="owner@scheduling.test", password="pw12345678")
    Membership.objects.create(user=user, company=company, role=Membership.OWNER)
    return user


def test_scheduling_index_renders(client, owner):
...
def test_scheduling_gateway_not_configured():
    from scheduling.gateway import configured
