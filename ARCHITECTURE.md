...
## Phase 4 — Money machine (contracts)

### billing/ changes (placement-linked pricing)
- Plans: STARTER price_monthly_inr 999 **per recruiter seat** (seat-based: `pricing_model` SEAT|FLAT on Plan; STARTER=SEAT, GROWTH/AGENCY=FLAT), `success_fee_inr` per hire (STARTER 4999, GROWTH 2999, AGENCY 0), `ai_included` (50/500/2000) and `ai_overage_inr` (5). FREE plan removed from choice for new companies: trial (14 days AGENCY) → STARTER unless paid; `expire_trials` downgrades to STARTER with a 7-day grace before hard-limit enforcement.
- PlacementFee is created for EVERY hire when plan.success_fee_inr > 0 (existing model; fix signal), grouped monthly onto the Invoice as line items (Invoice gains `line_items` JSON: [{kind: SUBSCRIPTION|SUCCESS_FEE|AI_OVERAGE|BGV|SEAT, label, qty, unit_inr, total_inr}]). `billing.invoicing.build_monthly_invoice(company, period)` and command `bill_month`.
- AI metering: `usage.consume` beyond `ai_included` records overage instead of raising, unless `hard_cap` flag on subscription.
- Overview: yearly default toggle, "your bill this month" projection card, success-fee ledger.

### contracting/ (staffing back office) — new app, feature flag `contracting` (GROWTH+AGENCY)
- Contractor(company, candidate FK CandidateProfile null, name, email, phone, pan, uan, bank JSON encrypted (reuse integrations.crypto), status ONBOARDING/ACTIVE/ENDED), Engagement(contractor, client FK clients.Client, job FK null, role_title, start, end null, bill_rate_inr, pay_rate_inr, rate_unit HOUR|DAY|MONTH, po_number, status), OnboardingDocument(contractor, kind PAN|AADHAAR|OFFER|NDA|OTHER, file, verified_at), Timesheet(engagement, period_start, period_end, entries JSON [{date, hours, note}], total_hours, status DRAFT/SUBMITTED/APPROVED/REJECTED/INVOICED, submitted_at, approved_by (client access or staff), approved_at, client_note), ClientInvoice(company, client, number sequential per FY like INV/2026-27/0001 via a locked counter, period, line_items JSON, subtotal, gst_rate, cgst/sgst/igst (reuse billing.invoicing.gst_split), total, status DRAFT/SENT/PAID/OVERDUE, due_at, pdf, sent_at, paid_at), PayrollRun(company, month, rows JSON [{contractor, days/hours, gross, tds, net}], csv FileField, status).
- Flows: contractor onboarding checklist; contractor self-service timesheet page via token link (reuse core.tokens.TokenMixin) or logged-in candidate; client approves timesheets inside the EXISTING client portal (clients/portal gets a "Timesheets" tab via an include partial that clients agent-owner adds: `contracting/partials/portal_timesheets.html`); month-end: approved timesheets → ClientInvoice per client (GST via billing helpers, PDF via xhtml2pdf) → send via notifications; PayrollRun computes gross from pay_rate, TDS at a configurable % (default 10% for contractors u/s 194J/194C selectable), exports CSV for Keka/greytHR (reuse integrations connectors' employee payload where possible).
- Metrics: gross margin per engagement (bill − pay), DSO on invoices, utilisation; surfaced on analytics via late import.

### exchange/ (agency requirement network) — new app, feature flag `exchange` (AGENCY, and any plan can RESPOND for free)
- PartnerLink(from_company, to_company, status PENDING/ACTIVE/BLOCKED, created_by) mutual trust; ExchangeRequirement(company, job FK, title, skills, location, budget_ctc_min/max, fee_split_pct default 50, visibility PARTNERS|NETWORK, status OPEN/FILLED/CLOSED, expires_at), ExchangeSubmission(requirement, responding_company, candidate_snapshot JSON (anonymised until accepted), resume_file, note, status SUBMITTED/SHORTLISTED/INTERVIEWING/HIRED/REJECTED, revealed_at), ExchangeDeal(submission, placement_fee_inr, split_pct, platform_fee_pct default 12, status PENDING/INVOICED/SETTLED) → platform invoice line via billing.
- Flows: publish a job to the exchange (anonymised client), partners see a feed, submit candidates from their talent pool or CandidateProfile, requester shortlists (reveal identity on accept), on HIRED the deal records fees; platform_fee becomes a billing line item for BOTH sides; disputes flag.
- Anti-leak: candidate contact hidden until accepted; duplicate detection by email hash across submissions; the requester's client name hidden unless visibility PARTNERS.

### Shared wiring (done by the main session): apps registered, urls at contracting/ and exchange/, sidebar links (Hiring ▸ Exchange; Company ▸ Contracting), pyproject testpaths, feature flags added to FEATURES and PLAN_SPECS by billing agent.

## Phase 5 — Opportunity network (contracts for parallel agents)

Two sides. **Seekers** (existing `User.is_candidate` accounts using the candidate
portal at `/portal/`) get one place to find work and reach out. **Tenants** get
a cross-tenant job board that brings them applicants. Recruiters' own jobs are
first-class inventory in the seeker feed.

Research findings (verified 2026-09-06, keep adapters to these):
- Work without auth: HN Algolia (`hn.algolia.com/api/v1`, monthly "Who is hiring" and "Freelancer? Seeking freelancer?" threads by author `whoishiring`; ~20% of freelancer comments carry an email), Remotive, Arbeitnow, RemoteOK (ToS: link back to the RemoteOK URL, no nofollow), Jobicy, Himalayas, WeWorkRemotely RSS, Freelancer.com projects API (budgets in INR available), ATS public boards: Greenhouse `boards-api.greenhouse.io/v1/boards/{slug}/jobs?content=true`, Lever `api.lever.co/v0/postings/{slug}?mode=json`, Ashby `api.ashbyhq.com/posting-api/job-board/{slug}` (can be 12MB; stream/limit), SmartRecruiters `api.smartrecruiters.com/v1/companies/{slug}/postings`.
- Needs credentials: Reddit (OAuth app; adapter is skipped when `REDDIT_CLIENT_ID`/`REDDIT_CLIENT_SECRET` unset), Adzuna (`ADZUNA_APP_ID`/`ADZUNA_APP_KEY`).
- Do NOT scrape: LinkedIn, Upwork (RSS returns 410), Naukri, Wellfound, Instahyre. These enter only via user capture (paste / bookmarklet).

Rules for every adapter: send a descriptive User-Agent; honour robots.txt and a per-source rate limit; store a **snippet (≤ 600 chars) and the source link**, never the full posting; extract a contact email only from text the poster wrote; never fetch in tests (record fixtures under `sources/tests/fixtures/`).

### sources/ — new app (agent A)
- `Source(slug unique, name, kind API|RSS|ATS, enabled bool, config JSON e.g. {"slug": "stripe"} for ATS boards, last_run_at, last_status, last_error, items_seen int)`. Seed migration: one row per public source above plus ~30 ATS boards (well-known companies hiring in India/remote; slugs verified in the seed by the agent).
- `Lead(source FK, external_id, kind JOB|FREELANCE|GIG, title, company_name, snippet, url unique-ish, contact_email blank, location, remote bool, salary_min/max null, currency, budget_text, tags JSON, skills JSON (normalised lower-case list), posted_at, fetched_at, content_hash unique (sha256 of normalised title+company+first 300 chars), is_active bool, expires_at)`; index on (kind, posted_at), (is_active, posted_at).
- `sources/adapters/base.py`: `class Adapter: slug; def fetch(self, source) -> Iterable[RawItem]`; `RawItem` dataclass with the Lead fields. One module per source in `sources/adapters/`. Registry dict `ADAPTERS`.
- `sources/services.py`: `run_source(source) -> stats`, `run_all()`, `normalise()`, `dedupe()` by content_hash, `extract_contact(text)` (RFC-ish email regex, ignores noreply/example domains), `tag_skills(lead)` using `core.llm.complete` in batches of 20 leads per call with a fixed skill vocabulary from `jobs.Skill` names plus a built-in list (python, django, react, …); LLM failure → keyword fallback. `expire_stale()` marks `is_active=False` after 45 days or when the source no longer returns it (ATS).
- Command `fetch_sources` (`--only slug`), added to `PERIODIC_COMMANDS` by the main session. Admin for Source and Lead with a "run now" action.
- Public service API for other apps: `sources.services.search_leads(query="", skills=None, kind=None, remote=None, since=None, limit=50)` returning a queryset; `sources.services.leads_for_profile(candidate_profile, limit=50)` ranking by skill overlap (Jaccard on skills) then recency.
- Tests: adapter parsing from fixtures for every adapter, dedupe, contact extraction (positive/negative), skill tagging fallback, command runs with network mocked.

### seeker/ — new app (agent B), lives inside the candidate portal
- Models: `SeekerProfile(user OneToOne, target_roles JSON, skills JSON (mirrors CandidateProfile.skills names), min_budget_inr null, remote_only bool, is_pro bool, sends_this_month int, month_key "YYYY-MM", mailbox_kind NONE|GMAIL|SMTP, mailbox_config encrypted JSON via integrations.crypto.encrypt_json (smtp host/port/user/password or gmail refresh token), mailbox_email)`. `SavedItem(seeker, lead FK sources.Lead null, job FK jobs.Job null, status SAVED|APPLIED|CONTACTED|REPLIED|REJECTED|ARCHIVED, note, created_at)` exactly one of lead/job set (CheckConstraint). `Outreach(seeker, saved_item, to_email, subject, body, resume_attached bool, status DRAFT|SENT|FAILED, provider_message_id, sent_at, error)`.
- Pages under `/portal/opportunities/` (namespace `seeker`): **Feed** (leads_for_profile + network jobs from `board.services.network_jobs_for(profile)`; filters kind/remote/skills/query; each card: title, company, snippet, source badge, match %, Save); **Saved** (multi-select → bulk action bar: "Apply" for internal jobs = call the existing `web.views.job_apply` logic via a service function extracted to `web/services/apply.py::apply_to_job(user, job)` — agent B may create that module and make `job_apply` call it; "Open & track" for external jobs (mark APPLIED after opening); "Draft emails" for leads with contact_email); **Compose** (per selected lead an AI draft via `core.llm.complete`: subject + body ≤ 180 words, references 2 concrete facts from the lead snippet and 2 from the seeker resume_text, plain-text, no placeholders; user edits then "Send N"); **Mailbox** settings (SMTP app-password form; Gmail OAuth using scope `https://www.googleapis.com/auth/gmail.send` when GOOGLE_OAUTH_* set — reuse the token exchange pattern in `scheduling/calendar/google.py`, do not import its calendar scopes); **Add a lead** (paste URL/text → creates a `sources.Lead` with source slug `manual`, plus a bookmarklet snippet on the page).
- Sending: always from the seeker's own mailbox; never via DEFAULT_FROM_EMAIL. Resume attached from `CandidateProfile.resume`. Free tier: 10 sends per calendar month; `is_pro` lifts it. Over quota → 402 page with upgrade CTA to `seeker:upgrade` (page exists; payment wiring is a stub that records intent — do not build billing here).
- Guardrails: one recipient per Outreach (no BCC lists), max 25 sends per bulk action, refuse if `to_email` was not extracted from the lead text or typed by the user on that lead, "unsubscribe" not required (1:1 mail) but include the seeker's name and reply-to.
- Tests: quota, constraint, drafting with llm mocked, SMTP send with `django.core.mail` locmem-style fake, bulk apply creates Applications exactly once, mailbox config round-trips encrypted.

### board/ — new app (agent C): cross-tenant public job board
- `CareersSite` gains `list_in_network = BooleanField(default=True)` (agent C adds the migration in careers/ — the only cross-app write allowed).
- Public pages at `/jobs/board/` (namespace `board`): list of every `Job` with status OPEN whose company has a published CareersSite with `list_in_network`; search (title/skills/location), filters (remote, salary shown when `Job.show_salary`), pagination; detail page reuses the careers job template look; **Apply** button → logged-in candidates call `web.services.apply.apply_to_job` (coordinate: agent B creates it; if absent when C runs, C creates the same function signature and B adopts it — identical module path `web/services/apply.py`, function `apply_to_job(user, job) -> Application`), anonymous users → candidate signup with `?next=`.
- SEO: JSON-LD `JobPosting` per detail page (reuse careers' builder), `/jobs/board/sitemap.xml`, `/jobs/board/feed.xml` (RSS).
- `board/services.py`: `network_jobs(query="", skills=None, remote=None)` and `network_jobs_for(candidate_profile, limit=50)` ranked by skill overlap — consumed by seeker feed.
- Tenant switch: Careers settings page gets a "List on the network" toggle (agent C adds a small include partial to careers index; keep the edit minimal).
- Tests: only OPEN jobs from published, opted-in sites appear; apply creates one Application; JSON-LD validates required fields.

### Shared wiring (main session): apps registered, urls (`sources` has no urls; `seeker` under `portal/opportunities/`; `board` under `jobs/board/`), `fetch_sources` in PERIODIC_COMMANDS, candidate portal nav link, pyproject testpaths, env keys documented in `.env.example`.
