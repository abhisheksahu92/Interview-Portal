...
def owner(db):
    ...
    user = User.objects.create_user(email="owner@offers.test", password="pw12345678")
    Membership.objects.create(user=user, company=company, role=Membership.OWNER)
    return user


def test_offers_index_renders(client, owner):
...
def test_offers_gateway_not_configured():
